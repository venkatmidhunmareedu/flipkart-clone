...
Project skills live in `.cursor/skills/`. Use them when the task matches:

- `flipkart-dev-setup` — install deps, env, run dev servers
- `add-api-endpoint` — scaffold Express routes/controllers
- `ecommerce-catalog` — products, categories, search, filters
- `ecommerce-cart-checkout` — cart, addresses, orders, payment flow
- `add-ui-page` — Next.js pages and shared UI components

## Taskmaster AI

Task-driven development via [Taskmaster](https://docs.task-master.dev). PRD at `.taskmaster/docs/prd.txt` (synced from `PRD.md`).

```bash
npx --package=task-master-ai task-master list              # view tasks
npx --package=task-master-ai task-master parse-prd .taskmaster/docs/prd.txt
npx --package=task-master-ai task-master next              # next task to work on
```

- Config: `.taskmaster/config.json`
- Tasks: `.taskmaster/tasks/`
- MCP: `.cursor/mcp.json` (enable **task-master-ai** in Cursor Settings → MCP)
- API keys: copy root `.env.example` → `.env` and fill at least one provider key
- Cursor slash commands: `.cursor/commands/tm/` (e.g. `/tm/parse-prd`)
